...
public class RandomSpawn
{
    ...
    public void Spawn(PlayerControl playerControl)
    {
        if (availableLocations == null || availableLocations.Count <= 0) ResetLocations();
        Utils.Teleport(playerControl.NetTransform, usedLocations[availableLocations!.PopRandom()]);
    }

    public MonoRpc SpawnDeferred(PlayerControl playerControl)
    {
        if (availableLocations == null || availableLocations.Count <= 0) ResetLocations();
        return Utils.TeleportDeferred(playerControl.NetTransform, usedLocations[availableLocations!.PopRandom()]);
    }
}